...
const BusinessForSale = ({locale}) => {
  ...
  useEffect(() => {
    const fetchVillas = async () => {
      try {
        const response = await fetch(`/api/listings?locale=${encodeURIComponent(locale)}&pageNumber=1&searchQuery=&listingStatus=Buy&propertyTypes=shop&priceRange=0,10000000&bedrooms=0&bathrooms=0&location=All%20Cities&squareFeet=&yearBuild=&categories=&currentSortingOption=Price%20High`); // Adjust the query params as needed
  ...
  return (
    <Swiper
      ...
      navigation={{
        nextEl: ".business_next__active",
        prevEl: ".business_prev__active",
      }}
      pagination={{
        el: ".business_pagination__active",
        clickable: true,
      }}
      breakpoints={{
        300: {
